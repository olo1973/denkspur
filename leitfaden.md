...
> Dieser Text führt in die Methode ein, wie man sie lernt, nicht wie man sie
> nachschlägt: erst das Problem, dann die Lösung, mit einem durchgängigen
> Beispiel. Die knappe Referenz steht in [`methode/`](methode/); die
> Kurzfassung für Agenten in [`AGENTS.md`](AGENTS.md). Maßgeblich ist
> `methode/` — Leitfaden und Agenten-Kurzfassung sind abgeleitete
> Darstellungen derselben Regeln.
...
1. **„Warum haben wir das nochmal so gebaut?"** Die Begründung steckte in
   einem Chat-Verlauf, einem Call oder einem Kopf und ist nicht mehr
   auffindbar. Also wird die Entscheidung neu aufgerollt. Das kostet Zeit
   und endet oft beim selben Ergebnis.
2. **„Wo liegt das nochmal?"** Die Ablage ist ungeplant gewachsen. Jede
   Suche kostet Minuten, und mit jeder neuen Datei wird es schlimmer.
...
erklärt bekommen. Daraus folgt die Grundregel: **Kontext gehört in die
Ablage, nicht in den Chat.**
...
- **Raum** (gegen Moment 2): eine vorhersehbare Struktur. Jede Datei hat
  einen Ort mit klarem Zweck, jedes Projekt eine Tür.
- **Zeit** (gegen Momente 1 und 3): eine Spur. Jeder tragende Gedanke
  durchläuft nachvollziehbare Stationen, und getroffene Entscheidungen
  werden nicht wegeditiert.
...
orientiert in fünf Minuten), **getrennte Lebensbereiche** (Rohes, Wissen und
Erzeugtes werden nicht gemischt) und **langweilige Namen** (kleingeschrieben,
...
Wir halten den Gedanken fest, bevor er im Alltag verloren geht, als
`ideen/0007-merkliste.md`:
...
Die Idee trägt, also befördern wir sie zu einem **Entscheidungs-Vorschlag**:
einem ADR (Architecture Decision Record, das bewährte Standardformat dafür),
abgelegt als `entscheidungen/0009-merkliste.md` mit Status `vorgeschlagen`:
...
entsteht ein neues ADR mit `Ersetzt: 0009`, und die Spur bleibt lückenlos.
...
Das **Design-Gate** löst Moment 4 aus Abschnitt 1: Weil die Merkliste
sichtbare Oberfläche hat, wird ihr Aussehen zuerst am billigsten tauglichen
Medium abgenommen, etwa einem Figma-Entwurf oder einem Bild-Mockup. Ein „der
...
als Bild unter `ergebnisse/abnahmen/` versioniert, denn ein bloßer
Figma-Link altert und ist für Dritte oft unzugänglich.
...
Feature ist jeder Schritt ein kleines Markdown-Artefakt, das beim Schreiben
wenig kostet und dauerhaft nachvollziehbar bleibt.

## 5. Den Überblick behalten

Zwei Hilfsdateien halten das Ganze übersichtlich:
...
- **`uebersicht.md`** — drei Tabellen (Ideen, Entscheidungen, Pläne) mit
  ...
  **komplett neu generiert**, nie von Hand geflickt. Quelle der Wahrheit
  bleiben allein die Artefakte, und die Fleißarbeit kostet weder Zeit noch
  Tokens.
...
1. **Gespräch** (Default): Der Agent schlägt frei vor, handelt aber erst auf
   ausdrückliche Freigabe. Weil ein bloßes „ja" mehrdeutig ist (gilt es dem
   einen Artefakt oder dem ganzen Paket?), benennt er vor dem Handeln, was
   er unter der Freigabe versteht.
...
- **Wegwerf-Arbeit:** Spikes, Experimente, Einmal-Skripte. Wo niemand
  wieder einsteigt, braucht es keine Spur.
- **Ein-Tages-Aufgaben** mit einer beteiligten Person und ohne
  Folgeentscheidungen — hier darf der Kontext im Chat-Verlauf bleiben.
...
- **Triviale, jederzeit umkehrbare Alltagsentscheidungen.** Ein ADR für
  jede Button-Farbe wäre reine Zeremonie.

Faustregel: Die Spur lohnt ab dem Moment, in dem eine zweite Person, eine
zweite Session oder ein zweiter Monat ins Spiel kommt. Die Methode skaliert
